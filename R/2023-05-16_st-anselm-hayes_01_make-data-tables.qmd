...
requested_shapes <- 
  as_tibble(requested_shapes) |> 
  select(name) |> 
  summarise(.by = name)

requested_shapes <- left_join(requested_shapes, intersections, by = "name")

head(requested_shapes)
```
...
```{r calculateStats}
requested_shapes <- group_by(requested_shapes, name)

shape_keys <-
  group_keys(requested_shapes) |> 
  unlist()

request_list <- 
  group_split(requested_shapes) |> 
...
all_areas_rel <- map(all_areas_rel, \(x) mutate(x, area = ordered(area,
                                                          levels = c("red brick estate", "townfield estate",
                                                                     "parish of st. anselm", "diocese", 
                                                                     "nation"))))

all_areas_abs <- map(all_areas_abs, \(x) mutate(x, area = ordered(area,
                                                          levels = c("red brick estate", "townfield estate",
                                                                     "parish of st. anselm", "diocese", 
                                                                     "nation"))))
...
saveRDS(all_areas_rel, here(save_dir, "2023-05-16_st-anselm-hayes_proportions.RDS"))
saveRDS(all_areas_abs, here(save_dir, "2023-05-16_st-anselm-hayes_population.RDS"))
```
