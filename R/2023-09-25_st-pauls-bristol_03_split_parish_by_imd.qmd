...
english_imd <- 
  ...
  filter(imd, str_starts(lsoa_2011_code, "E"))
...
## Join IMD vigintiles to LLSOA

### Create IMD vigintiles

```{r create_imd_vigintiles}
english_imd <- mutate(english_imd, imd_vigintile = ntile(imd_rank, 20))

## Check that's worked:
summarise(english_imd, .by = c(imd_decile, imd_vigintile)) |> 
  arrange(imd_vigintile) |> 
  head()
```

### Join vigintiles to LLSOA
